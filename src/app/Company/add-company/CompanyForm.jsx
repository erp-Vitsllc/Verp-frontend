...
import { COMPANY_ADD_MODULE, COMPANY_LIST_MODULE, notifyNoPermission } from '@/utils/companyPermissionModules';
...
export default function CompanyForm({ isEdit = false, companyId = '' }) {
    ...
    const canSetCompanyAddress = isAdmin() || hasPermission('hrm_company_view_basic_address', 'isCreate') || hasPermission('hrm_company_view_basic_address', 'isEdit');
    ...
    useEffect(() => {
        if (isEdit) {
            ...
            const hasEditPerm = isAdmin() || hasPermission(COMPANY_LIST_MODULE, 'isEdit');
            if (!hasEditPerm) {
                notifyNoPermission(toast, 'edit companies');
            ...
            const fetchCompanyData = async () => {
                try {
                    ...
                    if (comp.country) {
                        const foundCountry = Country.getAllCountries().find(
                            c => c.name.toLowerCase() === comp.country.toLowerCase() || 
                                 c.isoCode.toLowerCase() === comp.country.toLowerCase()
                        );
                        if (foundCountry) {
                            countryIso = foundCountry.isoCode;
                        }
                    }

                    // Convert state name/code to ISO code based on countryIso
                    let stateIso = '';
                    if (comp.state && countryIso) {
                        const foundState = State.getStatesOfCountry(countryIso).find(
                            s => s.name.toLowerCase() === comp.state.toLowerCase() || 
                                 s.isoCode.toLowerCase() === comp.state.toLowerCase()
                        );
                        if (foundState) {
                            stateIso = foundState.isoCode;
                        }
                    }

                    // Reconstruct phone
                    const fullPhone = (comp.phoneCountryCode || '') + (comp.phone || '');

                    setFormData({
                        name: comp.name || '',
                        companyId: comp.companyId || '',
                        establishedDate: comp.establishedDate ? comp.establishedDate.split('T')[0] : '',
                        email: comp.email || '',
                        phone: fullPhone,
                    ...
                    if (comp.phoneCountryCode) {
                        const cleanedCode = comp.phoneCountryCode.replace('+', '').toLowerCase();
                        setSelectedCountryCode(cleanedCode);
                    }
                } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEdit) {
            if (!isAdmin() && !hasPermission(COMPANY_LIST_MODULE, 'isEdit')) {
                notifyNoPermission(toast, 'edit companies');
                return;
            }
        } else {
        ...
        try {
            ...
            if (isEdit) {
                await axiosInstance.patch(`/Company/${companyId}`, submissionData);
                toast({
                    title: 'Success',
            ...
            toast({
                title: 'Error',
                description: err.response?.data?.message || `Failed to ${isEdit ? 'update' : 'register'} company`,
                variant: 'destructive',
            });
        } finally {
            setLoading(false);
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    <div className="flex-1 flex items-center justify-center p-8">
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-sm font-semibold text-slate-500">Loading company details...</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div className="flex items-center justify-between mb-10">
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <h1 className="text-3xl font-bold text-slate-800">{isEdit ? 'Edit Company Profile' : 'New Company Profile'}</h1>
                                </div>
                            </div>
                                    ...
                                    <div className="p-10 space-y-8 animate-in fade-in slide-in-from-right-10 duration-500">
                                        ...
                                        <div className="flex justify-end pt-8 mt-10 border-t border-slate-50">
                                            {canSetCompanyAddress ? (
                                                ...
                                                >
                                                    {loading ? 'Processing...' : (isEdit ? 'Update Company' : 'Register Company')}
                                                    <Check size={20} />
                                                </button>
                                            )}
                                        </div>
                                    ...
                                    <div className="p-10 space-y-8 animate-in fade-in slide-in-from-right-10 duration-500">
                                        ...
                                        <div className="flex items-center justify-between pt-8 mt-10 border-t border-slate-50">
                                            ...
                                            >
                                                ...
                                                {loading ? 'Processing...' : (isEdit ? 'Complete Update' : 'Complete Registration')}
                                            </button>
                                        </div>
                                    </div>
                                )}
