...
export default function CompanyProfilePage() {
    ...
    const [allCompaniesOwners, setAllCompaniesOwners] = useState([]); // [{name, company, ...ownerData}]
    ...
    // Fetch all owners across all companies for Trade License owner picker
    useEffect(() => {
        axiosInstance.get('/Company/all-owners')
            .then(res => {
                setAllCompaniesOwners(res.data?.owners || []);
            })
            .catch(() => { });
    }, []);
    ...
    useEffect(() => {
        if (activeTab === 'assets' && company?._id) {
            ...
            // 1. Find Asset Controller for this company to fetch Unassigned Assets
            const assetController = (company.responsibilities || []).find(r => r.category === 'assetcontroller' && r.status === 'Active');

            const fetchAssigned = axiosInstance.get('/AssetItem/assigned/all');
            const fetchUnassigned = assetController
                ? axiosInstance.get(`/AssetItem/unassigned/controller/${assetController.empObjectId}`)
                : Promise.resolve({ data: [] });

            Promise.all([fetchAssigned, fetchUnassigned])
                .then(([assignedRes, unassignedRes]) => {
                    const assigned = assignedRes.data || [];
                    const unassigned = unassignedRes.data || [];

                    const allAssets = [...assigned, ...unassigned];

                    const compName = (company.name || '').toLowerCase();
                    const compNick = (company.nickName || '').toLowerCase();
                    const compId = (company.companyId || '').toLowerCase();

                    const filtered = allAssets.filter(a => {
                        const assetName = (a.name || '').toLowerCase();
                        const assetId = (a.assetId || '').toLowerCase();

                        // Check if it's an employee of THIS company
                        const isFromCompany = a.assignedTo && (
                            String(a.assignedTo.company?._id || a.assignedTo.company) === String(company._id)
                        );

                        // Primary filter: asset must belong to the company (name/ID match)
                        const matchesCompanyName = assetName.includes(compName) ||
                            assetId.includes(compName) ||
                            (compNick && (assetName.includes(compNick) || assetId.includes(compNick))) ||
                            (compId && (assetName.includes(compId) || assetId.includes(compId)));

                        // Direct company assignment check
                        const isAssignedToThisCompany = a.assignedToType === 'Company' &&
                            String(a.assignedCompany?._id || a.assignedCompany) === String(company._id);

                        return matchesCompanyName || isAssignedToThisCompany;
                    });
    ...
    }, [activeTab, company?._id, company?.responsibilities, company?.name, company?.nickName, company?.companyId]);
    ...
    return (
        <div className="flex min-h-screen w-full bg-[#F2F6F9]">
            ...
            <div className="flex-1 flex flex-col min-w-0 relative overflow-x-hidden">
                ...
                {/* Tab Content */}
                <div className="bg-transparent min-h-[400px] pl-10 pr-8 pb-10">
                    {activeTab === 'basic' && (
                        <div className="animate-in fade-in duration-500 space-y-8">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full">
                                ...
                                {/* General No-Expiry Document Cards */}
                                {(company.documents || []).filter(d => (!d.expiryDate || d.expiryDate === '---') && !d.description?.toLowerCase().includes('previous') && !['trade license', 'establishment card', 'ejari', 'insurance'].some(t => d.type?.toLowerCase().includes(t))).map((doc, idx) => (
                                    <div key={`doc-no-expiry-${idx}`} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden h-fit">
                                        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
                                            <div className="flex flex-col">
                                                <h4 className="text-xl font-semibold text-gray-800">
                                                    {(doc.context === 'document_without_expiry' && doc.description && (!doc.type || doc.type.toLowerCase().includes('expiry')))
                                                        ? (typeof doc.description === 'string' ? doc.description : 'Legal Document')
                                                        : (typeof doc.type === 'string' ? doc.type : (doc.context === 'moa' ? 'MOA' : 'Legal Document'))}
                                                </h4>
                                                <span className="text-xs font-bold text-gray-400 uppercase tracking-tight mt-0.5">
                                                    {(doc.context === 'document_without_expiry' && doc.description && (!doc.type || doc.type.toLowerCase().includes('expiry')))
                                                        ? 'Document Without Expiry'
                                                        : (doc.context === 'moa' ? 'Permanent Record' : 'No Expiry Date')}
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {doc.document?.url && (
                                                ...
                                                <button
                                                    onClick={() => {
                                                        const actualIndex = (company.documents || []).indexOf(doc);
                                                        setEditingIndex(actualIndex);
                                                        handleModalOpen('companyDocument', actualIndex, 'document_without_expiry');
                                                    }}
                                                    className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                                                >
                                                    <Edit2 size={18} />
                                                ...
                                                <button
                                                    onClick={() => handleDeleteArrayItem('documents', (company.documents || []).indexOf(doc))}
                                                    className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-all"
                                                    title="Delete Card"
                                                >
                                                    <Trash2 size={18} />
                                        ...
                                        <div className="divide-y divide-slate-50">
                                            ...
                                            <div className="flex items-center justify-between px-8 py-4 hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <span className="text-sm font-bold text-emerald-600">{doc.value ? `${Number(doc.value).toLocaleString()} AED` : '---'}</span>
                                            </div>
                                            {doc.description && (
                                                <div className="flex flex-col px-8 py-4 hover:bg-gray-50/50 transition-colors">
                                                    <span className="text-xs font-bold text-gray-400 uppercase tracking-tight mb-1">Note</span>
                                                    <span className="text-sm font-medium text-gray-600">{doc.description}</span>
                                                </div>
                                            )}
                                            {doc.document?.url && (
                                                <div className="flex items-center justify-between px-8 py-4 hover:bg-slate-50/50 transition-colors">
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 min-h-[400px]">
                                <div className="flex items-center justify-between mb-6">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-400 mt-0.5">Assets belonging to this company (filtered by name/ID)</p>
                                    </div>
                                </div>
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {assetsLoading ? (
                                                ...
                                                <tr>
                                                    <td colSpan={8} className="px-6 py-20 text-center">
                                                        <div className="flex flex-col items-center gap-3">
                                                            ...
                                                            <span className="text-sm font-semibold text-gray-400">No assets found belonging to this company</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ) : (
                                                companyAssets.map((asset, idx) => {
                                                    ...
                                                    const empName = asset.assignedToType === 'Company'
                                                        ? (asset.assignedCompany?.name || 'Company Allocation')
                                ...
                                {modalType === 'tradeLicense' && (
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-3 pt-4 border-t border-gray-100">
                                            <div className="flex items-center justify-between">
                                                <label className="text-sm font-bold text-gray-700">Owners</label>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="space-y-3">
                                                {modalData.owners?.map((owner, index) => (
                                                    <div key={index} className="rounded-xl border border-gray-100 bg-gray-50/60 p-3 space-y-3">

                                                        {/* Top row: name + share + delete */}
                                                        <div className="flex items-center gap-3">
                                                            <div className="flex-1">
                                                                {owner.isNew ? (
                                                                    owner._useExisting ? (
                                                                        /* ── Existing owner chip ── */
                                                                        <div className="flex items-center gap-3 bg-white border border-blue-100 rounded-lg px-3 py-2 shadow-sm">
                                                                            ...
                                                                            <div className="flex-1 min-w-0">
                                                                                ...
                                                                                <p className="text-[10px] font-semibold text-blue-500 truncate">{owner._companyName}</p>
                                                                            </div>
                                                                            <button
                                                                                type="button"
                                                                                onClick={() => {
                                                                                    setModalData(prev => {
                                                                                        ...
                                                                                        owners[index] = { isNew: true, name: '', sharePercentage: owner.sharePercentage };
                                                                                        return { ...prev, owners };
                                                                                    });
                                                                                }}
                                                                                className="shrink-0 text-[10px] font-bold text-gray-400 hover:text-blue-600 border border-gray-200 hover:border-blue-300 px-2 py-1 rounded-md transition-all"
                                                                            >
                                                                                Change
                                                                            </button>
                                                                        </div>
                                                                    ) : owner._useExisting === false ? (
                                                                        /* ── Typing new name ── */
                                                                        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2.5 shadow-sm">
                                                                            <input
                                                                                ...
                                                                                className="flex-1 bg-transparent border-none p-0 focus:ring-0 text-sm font-semibold text-gray-800 placeholder:text-gray-300"
                                                                                autoFocus
                                                                            />
                                                                            <button
                                                                                type="button"
                                                                        ...
                                                                        <div className="flex gap-2">
                                                                            <button
                                                                                ...
                                                                                onClick={() => setModalData(prev => {
                                                                                    const owners = [...prev.owners];
                                                                                    owners[index] = { ...owners[index], _showPicker: true };
                                                                                    return { ...prev, owners };
                                                                                })}
                                                                                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg bg-white border border-blue-200 text-blue-600 text-xs font-bold hover:bg-blue-50 transition-all shadow-sm"
                                                                            >
                                                                                👤 Existing Owner
                                                                            </button>
                                                                            <button
                                                                                ...
                                                                                onClick={() => setModalData(prev => {
                                                                                    const owners = [...prev.owners];
                                                                                    owners[index] = { ...owners[index], _useExisting: false };
                                                                                    return { ...prev, owners };
                                                                                })}
                                                                                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg bg-white border border-gray-200 text-gray-600 text-xs font-bold hover:bg-gray-50 transition-all shadow-sm"
                                                                            >
                                                                                ✏️ New Owner
                                                                            </button>
                                                                        </div>
                                                                    ...
                                                                    /* ── Locked existing saved owner ── */
                                                                    <div className="flex items-center gap-3 bg-white border border-gray-100 rounded-lg px-3 py-2.5 shadow-sm">
                                                                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 flex items-center justify-center text-white text-xs font-black shrink-0">
                                                                            {owner.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                                                                        </div>
                                                                        <p className="text-sm font-bold text-gray-500 italic">{owner.name}</p>
                                                                    </div>
                                                                )}
                                                            </div>

                                                            {/* Share % */}
                                                            <div className="flex items-center gap-1 bg-white border border-gray-100 rounded-lg px-3 py-2 shadow-sm w-20 shrink-0">
                                                                <input
                                                                    type="number"
                                                            ...
                                                            <button
                                                                ...
                                                                className="w-9 h-9 flex items-center justify-center bg-red-50 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all shadow-sm shrink-0"
                                                            >
                                                                <Trash2 size={15} />
                                                            </button>
                                                        </div>

                                                        {/* Existing owner picker list */}
                                                        {owner._showPicker && (
                                                            <div className="rounded-xl border border-blue-100 bg-white shadow-md overflow-hidden">
                                                                <div className="px-3 py-2 border-b border-gray-50">
                                                                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Select from existing owners</p>
                                                                </div>
                                                                <div className="max-h-44 overflow-y-auto divide-y divide-gray-50">
                                                                    {allCompaniesOwners.length === 0 ? (
                                                                        <p className="text-xs text-gray-400 text-center py-4 italic">No existing owners found</p>
                                                                    ) : allCompaniesOwners.map((o, i) => (
                                                                        <button
                                                                            key={i}
                                                                            type="button"
                                                                            onClick={() => {
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <div className="flex-1 min-w-0">
                                                                                ...
                                                                                <p className="text-[10px] text-blue-400 font-semibold truncate">{o._companyName}</p>
                                                                            </div>
                                                                            {(o.passport?.number || o.visa?.number || o.emiratesId?.number) && (
                                                                                <span className="text-[9px] font-bold text-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded-full shrink-0">Has Docs</span>
                                                                            )}
                                                                        </button>
                                                                    ))}
                                                                </div>
