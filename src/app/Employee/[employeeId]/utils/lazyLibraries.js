...
/**
 * Lazy load react-phone-input-2 library
 * Only loads when phone input component is used
 * Size: ~80-100 KB
 */
export async function loadPhoneInput() {
    if (PhoneInputCache) {
        return PhoneInputCache;
    }
    ...
    try {
        ...
        if (typeof window !== 'undefined') {
            ...
            link.rel = 'stylesheet';
            link.href = '/node_modules/react-phone-input-2/lib/style.css';
            link.id = 'react-phone-input-2-css';

            // Check if already loaded
            if (!document.getElementById('react-phone-input-2-css')) {
                document.head.appendChild(link);
            }
        }

        const PhoneInput = (await import('react-phone-input-2')).default;
        PhoneInputCache = PhoneInput;
        return PhoneInputCache;
    } catch (error) {
        console.error('Failed to load react-phone-input-2:', error);
        throw error;
    }
}
