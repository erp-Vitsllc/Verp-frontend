...
const LabourCard = forwardRef(function LabourCard({
    ...
    const handleViewDocument = useCallback(async () => {
        ...
        const document = employee?.labourCardDetails?.document;
        if (!document) {
            toast({ variant: 'destructive', title: 'No document', description: 'No labour card document found.' });
            return;
        }
        await onViewDocument(
            employeeDocumentViewerPayload(document, {
                moduleId: labourPerm,
                defaultName: 'Labour_Card.pdf',
            }),
        );
    }, [employee, onViewDocument, toast, labourPerm]);
            ...
            return (
                <div className="rounded-2xl shadow-sm border break-inside-avoid mb-6 bg-white border-gray-100">
                    <div className="flex items-center px-3 sm:px-6 py-2.5 sm:py-4 border-b border-gray-100">
                        <h3 className="text-base sm:text-xl font-semibold text-gray-800">Labour Card</h3>
                    </div>
                    <p className="px-3 sm:px-6 py-2.5 sm:py-4 text-sm text-gray-500">No labour card on file.</p>
                </div>
            );
        }
        return (
