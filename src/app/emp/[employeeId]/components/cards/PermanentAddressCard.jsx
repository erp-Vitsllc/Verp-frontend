'use client';

import { crudAccess } from '@/utils/permissions';
import { canDeleteEmployeeCard } from '@/utils/employeeActivationSections';

const PERM = 'hrm_employees_view_permanent_address';

export default function PermanentAddressCard({
    employee,
    getCountryName,
    getStateName,
    hasPermanentAddress,
    onEdit,
    onDelete
}) {
    const access = crudAccess(PERM);
    const canDeleteAddress = canDeleteEmployeeCard(employee, access.delete, 'permanentAddress');

    if (!access.view) {
        return null;
    }

    if (!hasPermanentAddress) {
        if (!access.create && !access.edit) {
            return (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 break-inside-avoid mb-6">
                    <div className="flex items-center justify-between px-3 sm:px-6 py-2.5 sm:py-4 border-b border-gray-100">
                        <h3 className="text-base sm:text-xl font-semibold text-gray-800">Permanent Address</h3>
                    </div>
                    <p className="px-3 sm:px-6 py-2.5 sm:py-4 text-sm text-gray-500">No permanent address on file.</p>
                </div>
            );
        }
        return null;
    }

    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 break-inside-avoid mb-6">
            <div className="flex items-center justify-between px-3 sm:px-6 py-2.5 sm:py-4 border-b border-gray-100">
                <h3 className="text-base sm:text-xl font-semibold text-gray-800">Permanent Address</h3>
                {(access.edit || canDeleteAddress) && (
                    <div className="flex items-center gap-2">
                        {access.edit && (
                            <button
                                onClick={onEdit}
                                className="text-blue-600 hover:text-blue-700 transition-colors"
                                title="Edit"
                            >
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                </svg>
                            </button>
                        )}
                        {canDeleteAddress && onDelete && (
                            <button
                                onClick={onDelete}
                                className="text-red-600 hover:text-red-700 transition-colors"
                                title="Delete Permanent Address"
                            >
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                    <polyline points="3 6 5 6 21 6"></polyline>
                                    <path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"></path>
                                    <path d="M10 11v6"></path>
                                    <path d="M14 11v6"></path>
                                    <path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"></path>
                                </svg>
                            </button>
                        )}
                    </div>
                )}
            </div>
            <div>
                {[
                    { label: 'Address', value: employee.addressLine1 },
                    { label: 'Apartment/Flat', value: employee.addressLine2 },
                    { label: 'City', value: employee.city },
                    { label: 'Emirates/State', value: getStateName(employee.country, employee.state) },
                    { label: 'Country', value: getCountryName(employee.country) },
                    { label: 'ZIP Code', value: employee.postalCode }
                ]
                    .filter(row => row.value && row.value !== '—' && row.value.trim() !== '')
                    .map((row, index, arr) => (
                        <div
                            key={row.label}
                            className={`flex items-center justify-between px-3 sm:px-6 py-2.5 sm:py-4 text-sm font-medium text-gray-600 ${index !== arr.length - 1 ? 'border-b border-gray-100' : ''}`}
                        >
                            <span className="text-gray-500">{row.label}</span>
                            <span className="text-gray-500">{row.value}</span>
                        </div>
                    ))}
            </div>
        </div>
    );
}
