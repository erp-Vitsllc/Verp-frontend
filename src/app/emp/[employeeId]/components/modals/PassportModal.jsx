...
export default function PassportModal({
    ...
    const [isRenewal, setIsRenewal] = useState(isRenew); // Initialize with prop
    ...
    useEffect(() => {
        if (isOpen) {
            setIsRenewal(isRenew); // Sync with prop on open
            if (initialData) {
                setLocalForm({
            ...
            <div className="relative bg-white rounded-[22px] shadow-[0_5px_20px_rgba(0,0,0,0.1)] w-full max-w-[750px] max-h-[75vh] p-6 md:p-8 flex flex-col">
                <div className="flex items-center justify-center relative pb-3 border-b border-gray-200">
                    <h3 className="text-[22px] font-semibold text-gray-800">
                        {isRenewal ? 'Renew Passport' : 'Passport Details'}
                    </h3>
                    <button
                        onClick={onClose}
