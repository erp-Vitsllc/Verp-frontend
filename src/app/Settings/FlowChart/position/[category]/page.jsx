    ...
    return (
        <div className="flex h-screen bg-[#F8FAFC]">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <Navbar />
                <main className="flex-1 overflow-y-auto bg-slate-50/50 p-3 sm:p-5 lg:p-8">
                    <div className="max-w-5xl mx-auto">
                        ...
                        <div className="bg-white rounded-xl sm:rounded-2xl lg:rounded-[2rem] shadow-xl border border-slate-100 p-4 sm:p-6 lg:p-10">
                            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 mb-1 sm:mb-2">Position overview: {title}</h1>
                            ...
                            {loading && <p className="text-slate-400 font-bold text-xs sm:text-sm">Loading…</p>}
                            {err && <p className="text-red-600 font-bold text-xs sm:text-sm">{err}</p>}

                            {denied && data.viewerNote && (
                                <div className="rounded-xl sm:rounded-2xl border border-amber-200 bg-amber-50 text-amber-900 px-3 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm font-bold leading-relaxed">
                                    {data.viewerNote}
                                </div>
                            )}
                            ...
                            {!loading && !err && allowed && data && isCompanyAssetCoordinatorView(category) && (
                                <div className="space-y-4 sm:space-y-6 lg:space-y-8">
                                    <section>
                                        <h2 className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">
                                    ...
                                    <section>
                                        <h2 className="flex items-center gap-2 text-xs sm:text-sm font-black text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">
                                            <Building2 size={16} /> Company-allocated assets
                                        </h2>
                                        <div className="rounded-xl sm:rounded-2xl border border-slate-100 overflow-x-auto">
                                            <table className="w-full min-w-[480px] text-xs sm:text-sm">
                                                <thead className="bg-slate-50 text-left text-[10px] sm:text-xs font-black text-slate-500 uppercase">
                                                    <tr>
                                                        <th className="px-3 sm:px-4 py-2 sm:py-3">Asset ID</th>
                                                ...
                                                <tbody>
                                                    {(data.companyAssets || []).length === 0 ? (
                                                        <tr><td colSpan={4} className="px-3 sm:px-4 py-4 sm:py-6 text-slate-400 text-center">None</td></tr>
                                                    ) : (
                                                        (data.companyAssets || []).map((a) => (
                                                            <tr key={a._id} className="border-t border-slate-50">
                                                                <td className="px-3 sm:px-4 py-2 font-mono text-[10px] sm:text-xs">{a.assetId}</td>
                                                                <td className="px-3 sm:px-4 py-2">{a.name}</td>
                                                                <td className="px-3 sm:px-4 py-2">{a.status}</td>
                                                                <td className="px-3 sm:px-4 py-2 text-right">
                                                                    <Link href={getAssetDetailHref(a)} className="text-blue-600 font-bold text-[10px] sm:text-xs hover:underline">Open</Link>
                                                                </td>
                                                            </tr>
                                                        ))
                                                    )}
                            ...
                            {!loading && !err && allowed && data && normCat(category) === 'assetcontroller' && (
                                <div className="space-y-6 sm:space-y-8 lg:space-y-10">
                                    <section className="rounded-xl sm:rounded-[1.5rem] border-2 border-slate-200 bg-slate-50/40 p-3 sm:p-4 lg:p-6">
                                        <h2 className="flex items-center gap-2 text-xs sm:text-sm font-black text-slate-700 uppercase tracking-widest mb-1">
                                    ...
                                    <section className="rounded-xl sm:rounded-[1.5rem] border-2 border-amber-200/80 bg-amber-50/30 p-3 sm:p-4 lg:p-6">
                                        <h2 className="flex items-center gap-2 text-xs sm:text-sm font-black text-amber-900 uppercase tracking-widest mb-1">
                                            <Car size={18} className="text-amber-700" />
                                            Parking / On Leave
                                        </h2>
                                        <p className="text-[10px] sm:text-xs text-amber-900/70 mb-3 sm:mb-4 font-medium">
...
function AssetTableWithAccessories({ rows, showStatus }) {
    ...
    return (
        <div className="rounded-xl sm:rounded-2xl border border-slate-100 overflow-x-auto bg-white">
            <table className="w-full text-xs sm:text-sm min-w-[480px]">
                <thead className="bg-slate-50 text-left text-[10px] sm:text-xs font-black text-slate-500 uppercase">
                    <tr>
                        <th className="px-3 sm:px-4 py-2 sm:py-3 w-[28%]">Asset ID</th>
                        <th className="px-3 sm:px-4 py-2 sm:py-3">Name</th>
                        {showStatus && <th className="px-3 sm:px-4 py-2 sm:py-3 w-[14%]">Status</th>}
                        <th className="px-3 sm:px-4 py-2 sm:py-3 w-[12%] text-right"></th>
                    </tr>
                </thead>
                <tbody>
                    {rows.map((a) => {
