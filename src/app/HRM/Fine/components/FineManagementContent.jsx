...
const FineFlowManager = dynamic(() => import('./FineFlowManager'), { ssr: false });
const PendingFineRequestsModal = dynamic(() => import('./PendingFineRequestsModal'), { ssr: false });
...
function FineManagementContent({
    ...
    const [searchQuery, setSearchQuery] = useState(() =>
        embedded ? '' : (searchParams.get('search') || ''),
    );
    const [selectedFineType, setSelectedFineType] = useState(() =>
        embedded ? '' : (searchParams.get('fineType') || ''),
    );
    const [expandedGroups, setExpandedGroups] = useState({});
    const [activeTab, setActiveTab] = useState(() => {
        if (embedded) return 'individual';
        const tab = searchParams.get('tab');
        return tab === 'group' || tab === 'individual' ? tab : 'individual';
    });
    const [selectedStatus, setSelectedStatus] = useState(() =>
        embedded ? 'All' : (searchParams.get('status') || 'All'),
    );
    ...
    usePersistListReturnState(listReturnParams, !embedded);
    ...
    const fetchFines = useCallback(async () => {
        ...
        try {
            ...
            if (embedded && !vehicleId && !assetCode) {
                setFines([]);
                setLoading(false);
            ...
            const requests = [];
            if (vehicleId) {
                requests.push(axiosInstance.get('/Fine', { params: { limit: 1000, vehicleId } }));
            }
            if (assetCode) {
                requests.push(axiosInstance.get('/Fine', { params: { limit: 1000, assetId: assetCode } }));
            }
            if (!requests.length) {
                requests.push(axiosInstance.get('/Fine', { params: { limit: 1000 } }));
            }
            ...
            responses.forEach((response) => {
                const list = response.data.fines || response.data || [];
                (Array.isArray(list) ? list : []).forEach((fine) => {
            ...
            const typed =
                Array.isArray(allowedFineTypes) && allowedFineTypes.length
                    ? processed.filter((fine) => allowedFineTypes.includes(fine.fineType))
                    : processed;
            setFines(typed);
        } catch (err) {
    ...
    }, [vehicleId, assetCode, allowedFineTypes, embedded]);
    ...
    const inner = (
        <>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                            ...
                            <div className="min-w-0">
                                <h2 className="text-lg sm:text-xl font-bold text-gray-800">Vehicle Fines</h2>
                                <p className="text-xs sm:text-sm text-gray-500">
                                    Vehicle Fine and Vehicle Damage for this vehicle
                                </p>
                            </div>
                            )}
                        ...
                        <div className="flex items-center gap-4 sm:gap-6 lg:gap-10 mb-4 sm:mb-6 lg:mb-8 border-b border-gray-200 px-1 sm:px-2 overflow-x-auto">
                            <button
                                type="button"
                                {...(embedded ? {} : navHrefProps('/HRM/Fine'))}
                            ...
                            <button
                                type="button"
                                {...(embedded ? {} : navHrefProps('/HRM/Fine?tab=group'))}
...
export default FineManagementContent;
