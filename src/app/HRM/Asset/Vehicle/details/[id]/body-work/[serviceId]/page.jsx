...
import VehicleBodyWorkDetailHeaderCards from '@/app/HRM/Asset/Vehicle/components/VehicleBodyWorkDetailHeaderCards';
import VehicleBodyWorkDetailForm from '@/app/HRM/Asset/Vehicle/components/VehicleBodyWorkDetailForm';
...
import {
    resolveBodyWorkWorkflowStage,
    showBodyWorkGarageCard,
    showBodyWorkQuoteCard,
    showBodyWorkReturnCard,
    BODY_WORK_WORKFLOW_STAGES,
} from '@/app/HRM/Asset/Vehicle/utils/vehicleBodyWorkWorkflow';
...
const { page: bodyWorkPageLayout } = VEHICLE_HANDOVER_ASSIGN_WORKFLOW_TRACKER_CONFIG;

function VehicleBodyWorkDetailPageContent() {
    ...
    const load = useCallback(async ({ silent = false, light = false, deferServiceSigning = false } = {}) => {
        ...
        try {
            ...
            toast({
                variant: 'destructive',
                title: 'Could not load body work details',
    ...
    const service = useMemo(() => {
        ...
        return (
            services.find((row) => {
                ...
                return vehicleServiceTypeKey(row) === 'Body Work';
            }) || null
        );
    ...
    const bodyWorkflowStage = useMemo(
        () => resolveBodyWorkWorkflowStage(asset, serviceId, service),
    ...
    const canEditAssignment = assignmentPending
        ? Boolean(
              canCreateOrInitiate ||
                  (isVehicleServiceInitiateEditableStage(bodyWorkflowStage) &&
                      (canManageBodyWork || isFlowchartHr || isFlowchartAccounts)),
          )
        : isVehicleServiceInitiateEditableStage(bodyWorkflowStage)
          ? Boolean(canManageBodyWork || isFlowchartHr || isFlowchartAccounts)
          : canManageBodyWork;


    const canRespondToBodyWorkflow = useMemo(() => {
        if (!asset || bodyWorkflowStage !== BODY_WORK_WORKFLOW_STAGES.HR) return false;
        return asset.canRespondToServiceWorkflow === true;
    }, [asset, bodyWorkflowStage]);
                    ...
                    <div className="flex-1 flex flex-col items-stretch justify-start py-8 w-full px-6 md:px-8">
                        ...
                        <div className="flex flex-1 items-center justify-center">
                            <div className="flex flex-col items-center gap-3 text-slate-500">
                                ...
                                <span className="text-sm font-medium">Loading body work details...</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col items-stretch justify-start py-8 w-full px-6 md:px-8">
                        ...
                        <div className="bg-white rounded-lg p-12 text-center shadow-sm border border-gray-100">
                            ...
                            <h2 className="text-2xl font-bold text-gray-800 mb-2">Body work request not found</h2>
        ...
        <div className="flex min-h-screen w-full bg-[#F2F6F9]">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <div className="flex-1 flex flex-col items-stretch justify-start py-8 relative overflow-y-auto w-full px-6 md:px-8 print:py-0 animate-in fade-in duration-300">
                    ...
                    <VehicleBodyWorkDetailHeaderCards
                        vehicle={asset}
                        service={service}
                    ...
                    <div className={`${bodyWorkPageLayout.rowClassName} ${PAGE_SECTION_ANIMATION} delay-150`}>
                        ...
                        <div className={bodyWorkPageLayout.sideColumnClassName}>
                            <VehicleBodyWorkPreviousHistoryPanel
                                ...
                                className="shrink-0"
                            />
                            <VehicleBodyWorkDriverHistoryPanel
                                asset={asset}
                                service={service}
                                className="shrink-0"
                            />
...
export default function VehicleBodyWorkDetailPage() {
    return (
        <PermissionGuard moduleId="hrm_asset_vehicle" redirectTo="/dashboard">
            ...
            >
                <VehicleBodyWorkDetailPageContent />
            </Suspense>
        </PermissionGuard>
    );
}
