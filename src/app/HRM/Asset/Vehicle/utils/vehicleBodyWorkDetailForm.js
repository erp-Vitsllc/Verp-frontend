...
export { OIL_SERVICE_VENDOR_OPTIONS as BODY_WORK_VENDOR_OPTIONS, formatWarrantyExpiryFromAsset };
...
export function buildBodyWorkDetailFormState(service, asset, { flowchartRows = [] } = {}) {
...
export function validateBodyWorkDetailForm(formData, asset = null) {
...
export function getBodyWorkDetailFormMissingFields(formData, asset = null) {
    const errors = validateBodyWorkDetailForm(formData, asset);
    const labels = Object.keys(errors).map((key) => BODY_WORK_FIELD_LABELS[key] || errors[key]);
    return [...new Set(labels)];
}

export function isBodyWorkDetailFormComplete(formData, asset = null) {
    return Object.keys(validateBodyWorkDetailForm(formData, asset)).length === 0;
}

export function buildBodyWorkDetailSubmitBody(formData, { keepPending = true } = {}) {
    const liableOn =
