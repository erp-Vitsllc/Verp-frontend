...
function isCompletedMechanicalWorkServiceRecord(service, asset) {
    return isCompletedShopServiceHistoryRecord(service, asset);
}

export function buildMechanicalWorkPreviousHistoryDetailHref(vehicleId, serviceId) {
    return buildShopServiceHistoryDetailHref(vehicleId, serviceId, 'Mechanical Work');
}

export function buildMechanicalWorkPreviousHistoryEntries(asset, currentServiceId, { limit = 5 } = {}) {
    if (!asset) return [];
    ...
    return services
        ...
        .filter((s) => isCompletedMechanicalWorkServiceRecord(s, asset))
        ...
        .map((service) => {
            ...
            return {
                ...
                detailHref: buildMechanicalWorkPreviousHistoryDetailHref(asset._id, serviceId),
            };
        });
}
