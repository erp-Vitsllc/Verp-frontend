...
import { buildAccidentRepairPreviousHistoryDetailHref } from './vehicleAccidentRepairPreviousHistory';
...
function isCompletedAccidentRepairRow(row, asset) {
    const remark = parseVehicleServiceRemark({ remark: row?.remark }) || {};
...
export function resolveAccidentRepairCarDrivenBy(service, asset, employees = [], companies = []) {
    const remark = parseVehicleServiceRemark(service) || {};
    const drivenByType = String(remark.carDrivenByType || '').toLowerCase();
    const companyId = String(remark.carDrivenByCompanyId || '').trim();

    if (drivenByType === 'company' || companyId) {
        const match = (Array.isArray(companies) ? companies : []).find(
            (comp) => String(comp?._id || comp?.id || '') === companyId,
        );
        const name = match?.name || remark.carDrivenByCompanyName || 'Company';
        return { employeeId: '', companyId, name, isCompany: true };
    }
...
function mapHistoryEntry(row) {
    ...
    return {
        ...
        detailHref: buildAccidentRepairPreviousHistoryDetailHref(vehicleId, serviceId),
        sortDate: row.date ? new Date(row.date).getTime() : 0,
    };
}

export function buildAccidentRepairDriverHistoryEntries(
    fleetRows,
    employeeId,
    ...
    const consider = (row) => {
        if (!row) return;
        if (vehicleServiceTypeKey(row) !== 'Accident Repair') return;
        if (!rowMatchesDriver(row, driverMatchIds)) return;
        if (!isCompletedAccidentRepairRow(row, asset)) return;
