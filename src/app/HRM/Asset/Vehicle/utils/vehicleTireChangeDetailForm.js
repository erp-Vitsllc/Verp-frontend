...
export { OIL_SERVICE_VENDOR_OPTIONS as TIRE_CHANGE_VENDOR_OPTIONS, formatWarrantyExpiryFromAsset };
...
export function getLastCompletedTireServiceForAsset(asset, { excludeServiceId } = {}) {
    ...
    return (
        services
            .filter((s) => vehicleServiceTypeKey(s) === 'Tire Change')
...
export function buildTireChangeDetailFormState(service, asset, { flowchartRows = [] } = {}) {
...
const TIRE_CHANGE_FIELD_LABELS = {
...
export function validateTireChangeDetailForm(formData, asset = null) {
    ...
    // Description is optional on Tire Change details (initiate / schedule / HR / complete).
...
export function getTireChangeDetailFormMissingFields(formData, asset = null) {
    const errors = validateTireChangeDetailForm(formData, asset);
    const labels = Object.keys(errors).map((key) => TIRE_CHANGE_FIELD_LABELS[key] || errors[key]);
    return [...new Set(labels)];
}

export function isTireChangeDetailFormComplete(formData, asset = null) {
    return Object.keys(validateTireChangeDetailForm(formData, asset)).length === 0;
}

export function buildTireChangeDetailSubmitBody(formData, { keepPending = true } = {}) {
    const liableOn =
