...
function isCompletedAccidentRepairServiceRecord(service, asset) {
    return isCompletedShopServiceHistoryRecord(service, asset);
}

export function buildAccidentRepairPreviousHistoryDetailHref(vehicleId, serviceId) {
    return buildShopServiceHistoryDetailHref(vehicleId, serviceId, 'Accident Repair');
}

export function buildAccidentRepairPreviousHistoryEntries(asset, currentServiceId, { limit = 5 } = {}) {
    if (!asset) return [];
    ...
    return services
        ...
        .filter((s) => isCompletedAccidentRepairServiceRecord(s, asset))
        ...
        .map((service) => {
            ...
            return {
                ...
                detailHref: buildAccidentRepairPreviousHistoryDetailHref(asset._id, serviceId),
            };
        });
}
