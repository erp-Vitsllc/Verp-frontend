...
import { buildBodyWorkPreviousHistoryDetailHref } from './vehicleBodyWorkPreviousHistory';
...
function isCompletedBodyWorkRow(row, asset) {
    const remark = parseVehicleServiceRemark({ remark: row?.remark }) || {};
...
export function resolveBodyWorkCarDrivenBy(service, asset, employees = []) {
...
function mapHistoryEntry(row) {
    ...
    return {
        ...
        detailHref: buildBodyWorkPreviousHistoryDetailHref(vehicleId, serviceId),
        sortDate: row.date ? new Date(row.date).getTime() : 0,
    };
}

export function buildBodyWorkDriverHistoryEntries(
    fleetRows,
    employeeId,
    ...
    const consider = (row) => {
        if (!row) return;
        if (vehicleServiceTypeKey(row) !== 'Body Work') return;
        if (!rowMatchesDriver(row, driverMatchIds)) return;
        if (!isCompletedBodyWorkRow(row, asset)) return;
