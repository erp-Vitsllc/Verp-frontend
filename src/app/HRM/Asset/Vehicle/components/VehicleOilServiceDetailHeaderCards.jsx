...
import {
    resolveOilServiceApprovalStageLabel,
    resolveOilServiceHeaderStatus,
...
function buildVehicleSummaryFields(vehicle, service) {
    const remark = parseVehicleServiceRemark(service) || {};
    const stage = resolveOilServiceWorkflowStage(service, vehicle);
    ...
    if (stage === 'complete' && (service?.serviceDuration || wf.serviceDurationDays)) {
        fields.push({
            label: 'Service Duration',
            value:
                service?.serviceDuration ||
                `${wf.serviceDurationDays} day${wf.serviceDurationDays === 1 ? '' : 's'}`,
            tone: 'bg-teal-50 border-teal-100 text-teal-800',
        });
    } else if (remark.nextChangeKm) {
        fields.push({
            label: 'Next Service KM',
            value: String(remark.nextChangeKm),
            tone: 'bg-slate-50 border-slate-100 text-slate-800',
        });
    }

    fields.push(buildVehicleServiceTypeSummaryField(service, 'Oil Service'));

    return fields;
}

export default function VehicleOilServiceDetailHeaderCards({
    vehicle,
    service,
    isDraft = false,
    canRequest = false,
