...
import {
    ...
    getTireChangeDetailFormMissingFields,
    isTireChangeDetailFormComplete,
...
export default function VehicleTireChangeDetailForm({
    asset,
    service,
    vehicleId,
    serviceId,
    ...
    // Draft: create/initiate. After Send until Zoho billed: HR only (gated by canEditAssignment from page).
            ...
            try {
                ...
                if (submitAfterSave) {
                    ...
                    toast({
                        ...
                        description:
                            'Tire change assignment was sent. HR was emailed and will see this in the vehicle inbox bell.',
                    });
                    ...
                    toast({
                        ...
                        description: assignmentPending
                            ? 'Tire change assignment draft saved.'
    ...
    const handleSubmit = useCallback(async () => {
        if (!assignmentPending || !isTireChangeDetailFormComplete(formData, asset)) return;
        await persistForm({ submitAfterSave: true });
    }, [asset, assignmentPending, formData, persistForm]);

    const handleSaveDraft = async () => {
        if (!assignmentPending) {
            const blocking = getTireChangeDetailFormMissingFields(formData, asset);
