...
import {
    canEditTireChangeReturn,
    TIRE_CHANGE_WORKFLOW_STAGES,
} from '../utils/vehicleTireChangeWorkflow';
import {
    buildTireChangeReturnFormState,
    buildTireChangeReturnUpdateBody,
    createTireChangeOtherDocRow,
    isTireChangeReturnFormComplete,
...
export default function VehicleTireChangeReturnCard({
    ...
    const isComplete =
        stage === TIRE_CHANGE_WORKFLOW_STAGES.COMPLETE ||
    ...
    const addOtherDocRow = useCallback(() => {
        setFormData((prev) => ({
            ...prev,
            returnOtherDocs: [...(prev.returnOtherDocs || []), createTireChangeOtherDocRow()],
        }));
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        if (!isTireChangeReturnFormComplete(formData)) {
            const errors = validateTireChangeReturnForm(formData);
            toast({
                variant: 'destructive',
        ...
        setSaving(true);
        try {
            const body = buildTireChangeReturnUpdateBody(formData);
            const { data } = await axiosInstance.post(
                `/AssetItem/${vehicleId}/service/${serviceId}/tire-change/complete`,
                body,
            );
            toast({
