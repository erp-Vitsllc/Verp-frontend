...
import {
    buildAccidentRepairDriverHistoryEntries,
    resolveAccidentRepairCarDrivenBy,
} from '../utils/vehicleAccidentRepairDriverHistory';
...
export default function VehicleAccidentRepairDriverHistoryPanel({ asset, service, className = '' }) {
    const [fleetRows, setFleetRows] = useState([]);
    const [employees, setEmployees] = useState([]);
    const [loading, setLoading] = useState(true);

    const driver = useMemo(
        () => resolveAccidentRepairCarDrivenBy(service, asset, employees),
    ...
    const entries = useMemo(
        () =>
            buildAccidentRepairDriverHistoryEntries(fleetRows, driver.employeeId, {
                limit: 8,
                asset,
                employees,
            }),
        [fleetRows, driver.employeeId, asset, employees],
    );

    const title = driver.name
        ? `${driver.name} Completed Accident Repair Histories`
        : 'Driver Completed Accident Repair Histories';
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                        Completed accident repair services driven by this employee across all vehicles
                    </p>
                </div>
            </div>
            ...
            ) : !driver.employeeId && driver.isCompany ? (
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    Driver history is shown for employees only. {driver.name} is selected as a company.
                </p>
            ) : !driver.employeeId ? (
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    Select Who Committed Accident on the assignment form to see cross-vehicle accident repair history.
                </p>
            ) : !entries.length ? (
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    No completed accident repair records found for this driver across the fleet.
                </p>
            ) : (
