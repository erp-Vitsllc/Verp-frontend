...
import {
    buildBodyWorkDriverHistoryEntries,
    resolveBodyWorkCarDrivenBy,
} from '../utils/vehicleBodyWorkDriverHistory';
...
export default function VehicleBodyWorkDriverHistoryPanel({ asset, service, className = '' }) {
    const [fleetRows, setFleetRows] = useState([]);
    const [employees, setEmployees] = useState([]);
    const [loading, setLoading] = useState(true);

    const driver = useMemo(
        () => resolveBodyWorkCarDrivenBy(service, asset, employees),
    ...
    const entries = useMemo(
        () =>
            buildBodyWorkDriverHistoryEntries(fleetRows, driver.employeeId, {
                limit: 8,
                asset,
                employees,
            }),
    ...
    const title = driver.name
        ? `${driver.name} Completed Body Work Histories`
        : 'Driver Completed Body Work Histories';
        ...
        >
            ...
            >
                ...
                <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                        Completed body work services driven by this employee across all vehicles
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    Select Car Driven By on the assignment form to see cross-vehicle body work history.
                </p>
            ) : !entries.length ? (
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    No completed body work records found for this driver across the fleet.
                </p>
            ) : (
