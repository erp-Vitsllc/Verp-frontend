...
export default function VehicleAccidentRepairReturnCard({
    asset,
    service,
    vehicleId,
    serviceId,
    ...
    const isComplete =
        stage === ACCIDENT_REPAIR_WORKFLOW_STAGES.COMPLETE ||
    ...
    const handleSubmit = async () => {
        ...
        if (!isAccidentRepairReturnFormComplete(formData)) {
            const errors = validateAccidentRepairReturnForm(formData);
            toast({
                variant: 'destructive',
        ...
        setSaving(true);
        try {
            const body = buildAccidentRepairReturnUpdateBody(formData);
            const { data } = await axiosInstance.post(
                `/AssetItem/${vehicleId}/service/${serviceId}/accident-repair/complete`,
                body,
            );
            toast({
                title: 'Accident repair completed',
    ...
    const completeLocked =
        assignmentPending ||
        (!garageDone &&
            !['pending_admin_return', 'scheduled_service', 'pending_billing', 'billed', 'complete'].includes(
                stage,
            ) &&
            !isComplete);
    ...
    return (
        <>
            <div className={`w-full ${className}`.trim()}>
                <VehicleServiceLockedSection locked={completeLocked} message={completeLockMessage}>
                <FineFormCard
                ...
                >
                    ...
                    {canEditReturn ? (
                        <div className="mt-4 flex flex-wrap justify-end gap-3 border-t border-gray-100 pt-4">
                            ...
                            <button
                                type="button"
                                disabled={saving || fieldsDisabled || !isAccidentRepairReturnFormComplete(formData)}
