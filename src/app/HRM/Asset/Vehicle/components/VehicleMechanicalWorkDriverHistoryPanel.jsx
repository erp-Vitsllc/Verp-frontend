...
import {
    buildMechanicalWorkDriverHistoryEntries,
    resolveMechanicalWorkCarDrivenBy,
} from '../utils/vehicleMechanicalWorkDriverHistory';
...
export default function VehicleMechanicalWorkDriverHistoryPanel({ asset, service, className = '' }) {
    const [fleetRows, setFleetRows] = useState([]);
    const [employees, setEmployees] = useState([]);
    const [loading, setLoading] = useState(true);

    const driver = useMemo(
        () => resolveMechanicalWorkCarDrivenBy(service, asset, employees),
    ...
    const entries = useMemo(
        () =>
            buildMechanicalWorkDriverHistoryEntries(fleetRows, driver.employeeId, {
                limit: 8,
                asset,
                employees,
            }),
    ...
    const title = driver.name
        ? `${driver.name} Completed Mechanical Work Histories`
        : 'Driver Completed Mechanical Work Histories';
        ...
        >
            ...
            >
                ...
                <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                        Completed mechanical work services driven by this employee across all vehicles
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    Select Who Committed Accident on the assignment form to see cross-vehicle mechanical work history.
                </p>
            ) : !entries.length ? (
                <p className="flex flex-1 items-center justify-center py-10 text-center text-sm text-gray-500 px-4">
                    No completed mechanical work records found for this driver across the fleet.
                </p>
            ) : (
