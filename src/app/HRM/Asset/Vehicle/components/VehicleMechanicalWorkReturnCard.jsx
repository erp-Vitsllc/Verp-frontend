...
export default function VehicleMechanicalWorkReturnCard({
    asset,
    service,
    vehicleId,
    serviceId,
    ...
    const [formData, setFormData] = useState(() => buildMechanicalWorkReturnFormState(service, asset));
    ...
    const isComplete =
        stage === MECHANICAL_WORK_WORKFLOW_STAGES.COMPLETE ||
    ...
    const canEditReturn = canEditMechanicalWorkReturn(stage, canManage, isComplete, asset);
    ...
    useEffect(() => {
        setFormData(buildMechanicalWorkReturnFormState(service, asset));
    ...
    const handleSubmit = async () => {
        ...
        if (!isMechanicalWorkReturnFormComplete(formData)) {
            const errors = validateMechanicalWorkReturnForm(formData);
            toast({
                variant: 'destructive',
        ...
        setSaving(true);
        try {
            const body = buildMechanicalWorkReturnUpdateBody(formData);
            const { data } = await axiosInstance.post(
                `/AssetItem/${vehicleId}/service/${serviceId}/mechanical-work/complete`,
                body,
            );
            toast({
                title: 'Mechanical work completed',
    ...
    const missingFields = !fieldsDisabled && !isMechanicalWorkReturnFormComplete(formData)
        ? Object.values(validateMechanicalWorkReturnForm(formData))
        : [];

    // Plain call (not useMemo) — gate is cheap; avoids hooks-order crashes if render path changes.
