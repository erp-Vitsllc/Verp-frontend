...
export default function BulkPendingResolveModal({ isOpen, row, onClose, onSuccess }) {
    ...
    const isCreationBulk =
        row?.bulkKind === 'creation' || String(row?.requestType || '').trim() === 'Asset Approval';
    const selectedCount = useMemo(
        () => ids.filter((id) => checked.has(id)).length,
        [ids, checked],
    );
    const unselectedCount = ids.length - selectedCount;

    const helpText = isCreationBulk
        ? 'Checked assets will be approved. Unchecked assets are returned to Draft so the creator can edit and resubmit.'
        : 'Checked assets will be approved. Unchecked assets will be rejected. Use Check all / Uncheck all before Confirm.';
    ...
    const runSubmit = useCallback(async () => {
        ...
        try {
            ...
            const creation = row.bulkKind === 'creation' || row.requestType === 'Asset Approval';
            toast({
                title: 'Done',
                description:
                    selected.length && unselected.length
                        ? creation
                            ...
                            : `Approved ${selected.length} asset(s); rejected ${unselected.length}.`
                        : selected.length
                          ? `Approved ${selected.length} asset(s).`
                          : creation
                            ? `Returned ${unselected.length} asset(s) to draft.`
                            : `Rejected ${unselected.length} asset(s).`
            });
            onSuccess?.();
            onClose();
        } catch (e) {
    ...
    const requestLabel = String(row.requestType || row.extra2 || 'Bulk request').trim();
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-slate-200">
                <div className="flex items-start justify-between gap-3 p-5 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-[11px] font-bold text-amber-800 uppercase tracking-wide mt-1">{requestLabel}</p>
                        ...
                        <p className="text-xs font-semibold text-slate-500 mt-2">
                            {selectedCount} selected to approve
                            {unselectedCount > 0
                                ? ` · ${unselectedCount} will be ${isCreationBulk ? 'returned to draft' : 'rejected'}`
                                : ''}
                        </p>
                    </div>
                ...
                <div className="p-4 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/80">
                    ...
                    >
                        ...
                        {selectedCount === 0
                            ? isCreationBulk
                                ? `Return all to draft (${ids.length})`
                                : `Reject all (${ids.length})`
                            : unselectedCount === 0
                              ? `Approve all (${selectedCount})`
                              : `Approve ${selectedCount} · ${isCreationBulk ? 'Draft' : 'Reject'} ${unselectedCount}`}
                    </button>
                </div>
            </div>
        </div>
